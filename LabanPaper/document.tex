...
We denote our labled source's domain dataset as the tuple $(X_{source},
Y_{source})$ of lenght $m$, our unlabeled target's dataset of
lenght $n$ as $X_{target}$, and the concatination of $X_{source}$ and
...
designed several daily actions (greeting friends, playing with a baloon etc.),
and the CMAs commitie has tagged the clips. This dataset was small and
concetrated on the elemnts that are shown in table number \ref{nonCMAs}.
 \begin{table*}[ht]
	   \centering
	   \csvautotabular{nonCMAs.csv}
...
\bibitem{Bouchard}Bouchard, Durell, and Norman Badler. "Semantic segmentation
  ...
  \bibitem{LMA} http://www.laban-movement-analyses.be/
 \end{thebibliography}
\end{document}
