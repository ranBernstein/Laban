...
succeded getting over 60\% of recall and precision rate avarged over the
qualities.}
\end{abstract}
...
From that reason we narrowed our mission on the recognition of 18 Laban
...
system using a Bayesian reasoning framework \cite{Rett}. Zacharatos at al.
\cite{Zacharatos} and Kim at al. \cite{kim} got inspiration from LMA for emotion
recognition using the kinect sensor, but their focus was on the emotions, not
on the LMA.
...
\subsection{Per CMA evaluation}
	...
	\ref{oneCMASummary}. The performance on every Laban quality separetaly
...
the multitask from of Zou at al. \cite{Zou} regularuzation, where the
