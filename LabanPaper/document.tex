...
\title{Laban Movement Analysis using Kinect Sensor and Transfer Learning}
\maketitle
...
This paper presents an automated machine learning based method of recognition of
Laban qualities from motion capture skeletal recordings. The method is
demonstrated on the output of Microsoft's Kinect V2 sensor.}
\end{abstract}
...
\begin{itemize}
	...
	analysis, performing 18 clips each. Every clip is 3 seconds long too, where the
...
Base (SB).
This axe was approximated with the Kinect's horizontal axis(y), what made the
...
\subsection{Feature Selection}
	...
	\begin{itemize}
		...
		\begin{figure}[ht!]
			...
			selection. It can be seen that the optimal amount to select is 2\% 
			}
			\label{selection}
			\end{figure}
		\item
...
\subsubsection{Per quality performance in multitask setting}
	\begin{figure*}[ht]	
		\centering
	    ...
	    columns/Quality/.style={column type={|l|}},
     	columns/Precision/.style={column type={c}, column type/.add={}{|}},
     	columns/Recall/.style={column type={c}, column type/.add={}{|}},
     	columns/F1 score/.style={column type={c}, column type/.add={}{|}}, 
     	]{mixedSummary.csv}
...
As it was stated by Bickel at al. \cite{Bickel}, in the DA problem setting, a
...
Lets denote the tuple $(X_{source}, Y_{source})$ is our labled source dataset 
of lenght $m$, $X_{target}$ our unlabeled target dataset of lenght $n$, and the
concatination of the last two as $X$, and we will define $R_{(m+n) \times t}$
such as:
\[
    \R_{i,j}=
\begin{cases}
...
\end{cases}
\]
i.e. $r$ assigns 0 for every source sample and 1 for very target.
//
If $t$ is number of tasks that we handle and $d$ is our input dimension, except
to the mulititask classifier $W_{d \times t}$, we will learn an instance weighting $V_{d \times t}$ matrix, that will assign a weight for every instance in \testbf{every task}.
 when inserting $V$ to the multitask elastic
net framework, we get:
\\\textbf{Optimization Problem: }\textit{Over all W, and V, minimize:}
\begin{equation*}
	\|X_{source}V(Y_{source} - X_{source}W)\|^2_F + \lambda_1\cdot\|W\|_{2,1} +
	\lambda_2\cdot\|W\|^2_F + \lambda_3\cdot\|XV-R\|^2_F
\end{equation*}
i.e. learning both of the classifier and the instance weighting for all
of the tasks together.
\begin{thebibliography}{1}
\bibliographystyle{alpha}
\bibitem{Bickel}{Bickel, Steffen, Michael Brückner, and Tobias Scheffer.
