...
increasing in recent years as the preferred method for movement analysis. In
this study was developed a Machine Learning (ML) method for recognizing Laban
motor elements from marker-less Motion Capture (MOCAP) camera --- Microsoft's Kinect.
It obtained a recall and precision rate of about 60\%  averaged over the
elements.}
\end{abstract}
\section{Introduction}
The aim of our work is to create a method for an automated identification of Laban motor elements 
(might be referred also as qualities) that characterize any movement sequence,
using Kinect. Our problem is challenging in three aspects. The first is
quantifying subtle qualities that a well posed quantification for them haven't been found
yet. The second aspect is handling noisy sensory data with in-home setup, and the
...
background in movement if any. We propose a low-cost (100\$) , non-intrusive
(marker-less), ubiquitous (76 millions sensors around the world) system that can recognize Laban motor elements using the Kinect sensor and Software Development Kit (SDK).
...
we would be able to use all of the rich data that is provided by the Kinect, and not to focus on a very 
subtle feature extraction that requires domain expertise. Using ML gave us
an opportunity to do reverse engineering of the learned models, and learn about
our problem's intrinsic characteristics such as which features are predictive
to which qualities. The system obtained a recall and precision rate of between
...
or revealing of genetic tendency (phenotype) towards various illnesses such as Alzheimer,
autism, Parkinson's disease \cite{camurri2003application}, or schizophrenia,
based on analysis of the person’s motor behavior; monitoring influences over time and early detection of deviation from normal behavior in 
closed environments such as space travel, and more. Automated emotion recognition from movement is another 
...
(e.g., politicians and public speakers ; actors training) \cite{nguyen2012online}; or emotion recognition 
...
works is focused on LMA (not on emotions), but not using Kinect. Lourens et al.
\ref{lourens2010communicating} used video data and Samadani et al.
\ref{samadani2013laban} used a high quality MOCAP camera, but both of them
analyzed only hand guesters. Third line of works used Kinect as their main
sensor for skeletal information. Gabel et al.
\cite{gabel2012full} used Kinect for gait analysis. The work of
Zacharatos et al. \cite{Zacharatos} was inspired by LMA for emotion recognition
using Kinect. His feature extraction was influenced from LMA principles, but did
not attempt try to recognize the qualities themselves. Kim et al. \cite{kim}
tried to recognize the qualities themselves, but he didn't try his method on
real dataset and didn't evaluated the performance of his method.

\section{Method}
Due to the fact that we are the first to handle Laban recognition with
...
decided that most of the dataset will come from recording CMAs, and just a small
amount of validation clips will come from ordinary people. We didn't want to
constrain the lengths of the clips to be equal, so in order to get feature
vectors in uniform length (regardless of the original length of the clips),
...
and multitask learning (learning a model for the qualities all together).
...
\subsection{Feature Selection}
	...
	\begin{itemize}
		...
		\item
			...
			was selecting the top 60\% out of the features that have survived the first
...
We found some qualities are easier to recognize than other. The more
expressive ones such as \textit{jump}, \textit{spread}, and \textit{sink}, are
more quantifiable by our understanding and are easier to be recognized
(precision and recall of 60-90\%). The more subtle qualities such as
\textit{strong} and \textit{passive}, are more difficult for to quantify in
kinematic measurements, and for the system to obtain them. Overall we believe
that we succeeded capturing the essence of most of the qualities, and
gave a positive answer for the question if it can be done with a 100\$ sensor.
We believe that our work is a base and inspiration that will make the LMA method
