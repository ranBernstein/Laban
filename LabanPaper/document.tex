...
to the mulititask classifier $W_{d \times t}$, we will learn an instance
weighting $V_{m \times t}$ matrix, that will assign a weight for every instance in
\testbf{every task}.
Lets define $\Gamma_{(m+n)\times t}$ as a matrix such as:
\[
    \gamma_{i,j}=
\begin{cases}
    0,& \text{if } (x_i,y_i) \in L\\
    1,& \text{if } x_i \in T
\end{cases}
\]
So if our training set is $L=(X,Y)$, when inserting $V$ to the multitask elastic
net framework, we get:
\\\textbf{Optimization Problem: }\textit{Over all W, and V, minimize:}
\begin{equation*}
	\|V^T(Y - XW)\|^2_F + \lambda_1\cdot\|W\|_{2,1} + \lambda_1\cdot\|W\|^2_F +
	\lambda_3\cdot\|V-\Gamma\|^2_F
\end{equation*}
